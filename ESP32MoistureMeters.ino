...
const char *ssid = "";     //Your routers username
...
void loop() {
  ...
  int MonsteraHumidity = map(Analog1, wet1, dry1, 100, 0);     //Monstera: 40
  int PothosHumidity = map(Analog2, wet2, dry2, 100, 0);       //Pothos: 40
  int PitcherHumidity = map(Analog3, wet3, dry3, 100, 0);      //Pitcher: 55
  int VenusHumidity = map(Analog4, wet4, dry4, 100, 0);        //Venus: 50
  int AloeHumidity = map(Analog5, wet5, dry5, 100, 0);         //Aloe: 10
  int FalseAraliaHumidity = map(Analog6, wet6, dry6, 100, 0);  //False Aralia: 55
  // int  = map(Analog7, wet7, dry7, 100, 0);          //
  // int  = map(Analog8, wet8, dry8, 100, 0);        //
  // int  = map(Analog9, wet9, dry9, 100, 0);        //
  // int  = map(Analog10, wet10, dry10, 100, 0);     //
